...
## Purpose
Systematic evaluation of RAG systems using RAGAS (RAG Assessment Framework) with four complementary metrics for production-grade quality assurance.

## Concepts Covered
- RAGAS four metrics: faithfulness, answer relevancy, context precision, context recall
- Golden test set creation and versioning
- Domain-aware evaluation with thresholds (compliance, customer support, general)
- Automated evaluation pipelines with regression detection
- Batched processing with checkpoint recovery
- Cost analysis and decision frameworks for when to use/avoid RAGAS

## After Completing
You will be able to:
- Create representative golden test sets (100+ questions) for your domain
- Evaluate RAG systems systematically using RAGAS LLM-as-judge
- Detect regressions before production deployment (>5% drop alerts)
- Make informed decisions about evaluation approaches (RAGAS vs alternatives)
- Implement production-grade evaluation pipelines with cost controls

## Context in Track
- **Prerequisites:** Level 1 M4.3 (Basic validation), Level 1 M2.3 (Monitoring)
- **Current Module:** Module 8.1 - RAGAS Evaluation Framework
- **Next Module:** Module 8.2 - A/B Testing for RAG Systems
...
# Run tests
$env:PYTHONPATH="$PWD/src;$PWD"; pytest -q
...
- `OPENAI_API_KEY` - Required for RAGAS LLM-as-judge evaluation
- `SKIP_INTEGRATION_TESTS=true` - Skip tests requiring external services
- Domain-specific thresholds (FAITHFULNESS_THRESHOLD, etc.)
- Cost limits (MAX_MONTHLY_COST_USD, ALERT_THRESHOLD_COST_USD)
