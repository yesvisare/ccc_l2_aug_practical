...
from l2_m7_custom_business_metrics import (
    QueryMetrics,
    UserCohort,
    QueryAccuracy,
    FeatureType,
...
app = FastAPI(
    title="Module 7.3: Custom Business Metrics API",
...
class Hallucination RateRequest(BaseModel):
...
@app.post("/metrics/hallucination_rate")
...
@app.post("/metrics/feature_success")
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            "health": "/health",
            "record_query": "POST /metrics/query",
            "hallucination_rate": "POST /metrics/hallucination_rate",
            "active_users": "POST /metrics/active_users",
            "cost": "POST /metrics/cost",
            "feature_success": "POST /metrics/feature_success",
            "kpi_summary": "POST /kpi/summary",
...
async def startup_event():
    ...
    logger.info(f"Redis: {'Available' if REDIS_AVAILABLE else 'Not configured'}")
    logger.info(f"ClickHouse: {'Available' if CLICKHOUSE_AVAILABLE else 'Not configured'}")
